...
import * as codepages from '../codePageTable';
import { ConfigureTrigger } from "../cmakeTools";
import { CompileCommand } from '../compilationDatabase';
import { ConfigurationReader, defaultNumJobs } from '../config';
...
import { ProgressMessage } from '../drivers/cmakeServerClient';
import * as expand from '../expand';
//import { CMakeGenerator, effectiveKitEnvironment, Kit, kitChangeNeedsClean, KitDetect, getKitDetect, getVSKitEnvironment } from '../kit';
...
import { majorVersionSemver, minorVersionSemver, parseTargetTriple, TargetTriple } from '../triple';
import * as preset from '../preset';
import * as codeModel from '../drivers/codeModel';
import { DiagnosticsConfiguration } from '../folders';
import { Environment, EnvironmentUtils } from '../environmentVariables';
import { CustomBuildTaskTerminal } from '../cmakeTaskProvider';
import { getValue } from '../preset';
   ...
   // private _kitEnvironmentVariables = EnvironmentUtils.create();
    ...
    async getConfigureEnvironment(configurePreset?: preset.ConfigurePreset | null, extraEnvironmentVariables?: Environment): Promise<Environment> {
        ...
        if (this.useCMakePresets) {
            ...
            envs = null //this._kitEnvironmentVariables;
    ...
    async getCTestCommandEnvironment(): Promise<Environment> {
        if (this.useCMakePresets) {
            ...
            let envs = null //this._kitEnvironmentVariables;
            envs = EnvironmentUtils.merge([envs, await this.computeExpandedEnvironment(this.config.environment, envs)]);
...
*/
    ...
    get expansionOptions(): expand.ExpansionOptions {
        ...
        const vars: expand.KitContextVars = {
            ...
            buildKitVersionMajor: '0.0.0',//majorVersionSemver(version),
            buildKitVersionMinor: '0.0.0',//minorVersionSemver(version),
  ...
  //  protected abstract doSetKit(cb: () => Promise<void>): Promise<void>;
    ...
    private generateCMakeSettingsFlags(): string[] {
        ...
        if (toolchain.isValid) {
            settingMap[`CMAKE_C_COMPILER`] = { type: 'FILEPATH', value: toolchain.clangPath } as util.CMakeValue
            settingMap[`CMAKE_CXX_COMPILER`] = { type: 'FILEPATH', value: toolchain.clangcppPath } as util.CMakeValue
        }
    ...
    private isTargetsClean(targets?: string[]): Boolean {
        return (targets.length == 1) && (targets[0] == "clean")
    }
    ...
    async generateBuildCommandFromSettings(targets?: string[]): Promise<proc.BuildCommand | null> {
        ...
        if (numJobs && ((gen && !/Visual Studio/.test(gen)) || numJobs > 1)) {
            ...
            if (this.cmake.version && util.versionGreaterOrEquals(this.cmake.version, util.parseVersion('3.14.0'))) {
                ...
                if (gen) {
                    if (/(Unix|MinGW) Makefiles|Ninja/.test(gen) && !this.isTargetsClean(targets)) {
                        buildToolArgs.push('-j', numJobs.toString());
                    } else if (/Visual Studio/.test(gen) && !this.isTargetsClean(targets)) {
