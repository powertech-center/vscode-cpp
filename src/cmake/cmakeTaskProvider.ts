...
import { getCMakeToolsForActiveFolder } from './cmake';
...
export class CustomBuildTaskTerminal implements vscode.Pseudoterminal, proc.OutputConsumer {
    ...
    private async correctTargets(cmakeTools: CMakeTools, commandType: CommandType): Promise<string[]> {
        ...
        if (commandType === CommandType.install) {
            ...
            targets = ['clean'];
        } else if (!shouldIgnore && !targetIsDefined && !cmakeTools.useCMakePresets) {
            targets = [await cmakeTools.buildTargetName() || await cmakeTools.allTargetName];
        }
        return targets;
    }
