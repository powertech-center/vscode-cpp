...
class TypeHierarchyTreeItem extends vscode.TreeItem {
  constructor(item: TypeHierarchyItem) {
    ...
    this.command = {
      arguments: [item],
      command: 'cpp.typeHierarchy.gotoItem',
      title: 'Go to'
    };
  }
}
...
class TypeHierarchyFeature implements vscodelc.StaticFeature {
  ...
  private recomputeEnableTypeHierarchy() {
    if (this.state === vscodelc.State.Running) {
      vscode.commands.executeCommand('setContext', 'cpp.enableTypeHierarchy',
...
class TypeHierarchyProvider implements
  ...
  constructor(context: ClangdContext) {
    ...
    context.subscriptions.push(vscode.commands.registerTextEditorCommand(
        'cpp.typeHierarchy', this.reveal, this));
    context.subscriptions.push(vscode.commands.registerCommand(
        'cpp.typeHierarchy.close', this.close, this));
    context.subscriptions.push(vscode.commands.registerCommand(
        'cpp.typeHierarchy.gotoItem', this.gotoItem, this));
    context.subscriptions.push(vscode.commands.registerCommand(
        'cpp.typeHierarchy.viewParents',
        () => this.setDirection(TypeHierarchyDirection.Parents)));
    context.subscriptions.push(vscode.commands.registerCommand(
        'cpp.typeHierarchy.viewChildren',
        () => this.setDirection(TypeHierarchyDirection.Children)));

    this.treeView = vscode.window.createTreeView('cpp.typeHierarchyView',
  ...
  private async reveal(editor: vscode.TextEditor) {
    ...
    // "when": "extension.vscode-cpp.typeHierarchyVisible" from
    // package.json to evaluate to true.
    vscode.commands.executeCommand('setContext', 'cpp.typeHierarchyVisible',
                                   true);
